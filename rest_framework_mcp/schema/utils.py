...
def end_sentence(text: str) -> str:
    ...
    ends in punctuation (django-restql's "`items` field is not found" does not,
    DRF's own "This field is required." does), and a sentence appended to one
    that has not ended reads as a run-on to the model it is written for.
    """
