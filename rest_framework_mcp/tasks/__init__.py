...
The seam is deliberately narrow — :class:`TaskExecutor` has one method taking
one string — so Celery, RQ, Dramatiq or a thread pool all satisfy it and none
of them is imported here.

⚠ **``create_task`` and ``run_task`` are deliberately not re-exported here**,
and the omission is structural rather than an oversight. ``MCPCallContext``
declares a ``TaskStore``, so importing any task *type* executes this module —
and anything this module pulls in is therefore pulled into the middle of the
handler stack's own import. ``run_task`` imports the ``tools/call`` handler and
``create_task`` reaches ``transport``; either one closes a cycle.

Import them from their modules when you need them. Neither is the normal entry
