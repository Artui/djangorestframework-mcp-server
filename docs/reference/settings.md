...
| `ENFORCE_AUDIENCE` | `False` | Whether a token whose bound resource doesn't equal `RESOURCE_URL` is **rejected**. The MCP `2026-07-28` spec makes this validation a **MUST**, so treat the default as a floor rather than a recommendation: **django-oauth-toolkit 3.4.0+ records the resource on stock `AccessToken`**, and turning this on needs nothing else. It stays off by default despite the `[oauth]` extra flooring DOT at `>=3.4`, because a token that records *no* resource is rejected and only a client sending RFC 8707's `resource` parameter records one — an obligation MCP clients have and the other clients of your authorization server do not, so defaulting `True` would 401 them on upgrade. The check is capability-based, not version-based. With a swapped model without the field, or a DOT installed outside this extra and pinned older, use `OAUTH2_PROVIDER["ACCESS_TOKEN_MODEL"]` carrying a `resource` field or `DjangoOAuthToolkitBackend(audience_getter=…)`; without a route to the resource the backend raises `ImproperlyConfigured` at startup rather than 401-ing every request. Leaving it off on a deployment that *could* enforce raises `UnenforcedAudienceWarning` at construction. See [Authentication](../auth.md#audience-binding-rfc-8707). |
...
| `MAX_PAGE_SIZE` | `100` | Ceiling on how many rows one selector-tool call returns, paged or not. For a `paginate=True` tool it bounds the model-supplied `limit`, and clamps `page` to the last page that exists so an arbitrarily large page number cannot become an arbitrarily large SQL `OFFSET`. For a `paginate=False` tool it is applied as a `LIMIT` before rendering and the call is **refused** over the ceiling rather than truncated, for the reason `REQUIRE_LIST_PAGINATION` gives below. `None` disables the ceiling on both. Advertised as `maximum` on the generated `inputSchema` *and* clamped at dispatch — the schema tells a well-behaved model what to ask for, the clamp is what stops you trusting it. Clamping is safe here because `totalPages` / `hasNext` keep a clamped page self-describing. The default matches the page a selector tool serves when the model omits `limit` entirely, so an unconfigured deployment never advertises a page larger than the one it defaults to. |
...
| `REQUIRE_LIST_PAGINATION` | `False` | Registering a LIST selector tool with `paginate=False` raises `ImproperlyConfigured` instead of emitting `UnboundedListWarning`. Such a tool serialises whatever its selector resolves to — the whole table, for a plain `Model.objects.all()` — and unlike a paginated tool there is no honest way to clamp it, because the result carries no metadata saying rows were dropped. `MAX_PAGE_SIZE` is the backstop — applied as a `LIMIT` before the rows are rendered, so an oversized table is refused rather than fetched and measured, which is what `MAX_RESULT_BYTES` alone could not do. `paginate=True` is the fix. |
...
| Key | Default | What it does |
|---|---|---|
| `DCR_ENABLED` | `False` | Default for `build_oauth_urlpatterns(dcr_enabled=)`. RFC 7591 dynamic client registration; `False` makes `/oauth/register/` refuse every request with 403. An open DCR endpoint lets anyone create an OAuth client against your authorization server — one with no owning user, which is why only the `authorization_code` grant is registerable here. **DCR is deprecated by MCP revision `2026-07-28`** in favour of Client ID Metadata Documents, with an earliest removal of the first revision released on or after 2027-07-28; prefer `OAUTH2_PROVIDER["CIMD_ENABLED"]` and turn this on only for clients that cannot use it. See [Authentication](../auth.md#oauth-contrib-mount). |
