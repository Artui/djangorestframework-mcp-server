...
| `ENFORCE_AUDIENCE` | `False` | Whether a token whose bound resource doesn't equal `RESOURCE_URL` is **rejected**. The MCP `2026-07-28` spec makes this validation a **MUST**, so treat the default as a floor rather than a recommendation: **django-oauth-toolkit 3.4.0+ records the resource on stock `AccessToken`**, and turning this on needs nothing else. It stays off by default only because the `[oauth]` extra floors DOT at `>=2.3`, where no token records a resource and `True` would reject every request. The check is capability-based, not version-based. On older DOT, or a swapped model without the field, use `OAUTH2_PROVIDER["ACCESS_TOKEN_MODEL"]` carrying a `resource` field or `DjangoOAuthToolkitBackend(audience_getter=…)`; without a route to the resource the backend raises `ImproperlyConfigured` at startup rather than 401-ing every request. Leaving it off on a deployment that *could* enforce raises `UnenforcedAudienceWarning` at construction. See [Authentication](../auth.md#audience-binding-rfc-8707). |
